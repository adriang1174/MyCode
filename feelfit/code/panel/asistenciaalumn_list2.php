<?php
//Include Common Files @1-0B7A80FE
define("RelativePath", ".");
define("PathToCurrentPage", "/");
define("FileName", "asistenciaalumn_list2.php");
...
//Class_Initialize Event @2-9B1F61C2
...
"FROM alumno {SQL_Where} {SQL_OrderBy}";
            ...
            $this->s_fdesde = & new clsControl(ccsTextBox, "s_fdesde", "s_fdesde", ccsDate, array("dd", "/", "mm", "/", "yyyy"), CCGetRequestParam("s_fdesde", $Method, NULL), $this);
            $this->s_fhasta = & new clsControl(ccsTextBox, "s_fhasta", "s_fhasta", ccsDate, array("dd", "/", "mm", "/", "yyyy"), CCGetRequestParam("s_fhasta", $Method, NULL), $this);
...
//Operation Method @2-9254D38A
    function Operation()
    {
        ...
        $Redirect = "asistenciaalumn_list2.php";
        if($this->Validate()) {
            if($this->PressedButton == "Button_DoSearch") {
                $Redirect = "asistenciaalumn_list2.php" . "?" . CCMergeQueryStrings(CCGetQueryString("Form", array("Button_DoSearch", "Button_DoSearch_x", "Button_DoSearch_y")));
...
//Prepare Method @5-B876C1BE
    function Prepare()
    {
        ...
        $this->wp->Criterion[1] = $this->wp->Operation(opEqual, "rep_asistenciaalumno.idAlumno", $this->wp->GetDBValue("1"), $this->ToSQL($this->wp->GetDBValue("1"), ccsInteger),false);
        $this->wp->Criterion[2] = $this->wp->Operation(opGreaterThanOrEqual, "rep_asistenciaalumno.fecha", $this->wp->GetDBValue("2"), $this->ToSQL($this->wp->GetDBValue("2"), ccsDate),false);
        $this->wp->Criterion[3] = $this->wp->Operation(opLessThanOrEqual, "rep_asistenciaalumno.fecha", $this->wp->GetDBValue("3"), $this->ToSQL($this->wp->GetDBValue("3"), ccsDate),false);
...
//Open Method @5-212BFB6C
    function Open()
    {
        $this->CCSEventResult = CCGetEvent($this->CCSEvents, "BeforeBuildSelect", $this->Parent);
        $this->CountSQL = "SELECT COUNT(*)\n\n" .
        "FROM ((rep_asistenciaalumno INNER JOIN alumno ON\n\n" .
        "alumno.id = rep_asistenciaalumno.idAlumno) RIGHT JOIN equipamiento ON\n\n" .
        "equipamiento.id = rep_asistenciaalumno.idEquipamiento) INNER JOIN instructor ON\n\n" .
        "rep_asistenciaalumno.idInstructor = instructor.id";
        $this->SQL = "SELECT rep_asistenciaalumno.*, alumno.nombre AS alumno_nombre, descripcion, instructor.nombre AS instructor_nombre \n\n" .
        "FROM ((rep_asistenciaalumno INNER JOIN alumno ON\n\n" .
        "alumno.id = rep_asistenciaalumno.idAlumno) RIGHT JOIN equipamiento ON\n\n" .
        "equipamiento.id = rep_asistenciaalumno.idEquipamiento) INNER JOIN instructor ON\n\n" .
        "rep_asistenciaalumno.idInstructor = instructor.id {SQL_Where} {SQL_OrderBy}";
...
$FileName = FileName;
$Redirect = "";
$TemplateFileName = "asistenciaalumn_list2.html";
$BlockToParse = "main";
...
//Include events file @1-8F317004
include_once("./asistenciaalumn_list2_events.php");
